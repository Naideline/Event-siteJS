...
export function toggleInterested(event, interestedButton, goingButton) {
  const interestedEvents = getItemsFromLocalStorage("interestedEvents");
  const goingEvents = getItemsFromLocalStorage("goingEvents");
  const eventIndex = interestedEvents.findIndex(
    (interestedEvent) => interestedEvent.id === event.id,
  );
  ...
  if (eventIndex > -1) {
    ...
    const goingIndex = goingEvents.findIndex(
      (goingEvent) => goingEvent.id === event.id,
    );
    if (goingIndex > -1) {
...
export function toggleGoing(event, interestedButton, goingButton) {
  const interestedEvents = getItemsFromLocalStorage("interestedEvents");
  const goingEvents = getItemsFromLocalStorage("goingEvents");
  const eventIndex = goingEvents.findIndex(
    (goingEvent) => goingEvent.id === event.id,
  );
  ...
  if (eventIndex > -1) {
    ...
    const interestedIndex = interestedEvents.findIndex(
      (interestedEvent) => interestedEvent.id === event.id,
    );
